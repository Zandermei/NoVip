...
import zd.cn.novipvideo.utils.AudioManagerUtl;
import zd.cn.novipvideo.widget.constants.PlayParameter;
import zd.cn.novipvideo.widget.control.ControlView;
import zd.cn.novipvideo.widget.gesture.GestureDialogManager;
import zd.cn.novipvideo.widget.gesture.GestureView;
import zd.cn.novipvideo.widget.guide.GuideView;
import zd.cn.novipvideo.widget.interfaces.ViewAction;
import zd.cn.novipvideo.widget.listener.LockPortraitListener;
import zd.cn.novipvideo.widget.listener.OnAutoPlayListener;
import zd.cn.novipvideo.widget.listener.OnChangeQualityListener;
import zd.cn.novipvideo.widget.listener.OnScreenCostingSingleTagListener;
import zd.cn.novipvideo.widget.listener.OnStoppedListener;
...
public class AliyunVodPlayerView extends RelativeLayout implements ITheme {
    ...
    private OnVolumUpdateListener onVolumUpdateListener = null;//音量回调 zdd
    ...
    private void initGestureView() {
        ...
        mGestureView.setOnGestureListener(new GestureView.GestureListener() {
            ...
            public void onRightVerticalDistance(float downY, float nowY) {
                ...
                float volumes = AudioManagerUtl.getAudioManager().getStreamVolume(AudioManager.STREAM_MUSIC);
                AppLog.e("滑动音量默认："+volume);
                AppLog.e("滑动音量默认11："+volumes);
                AppLog.e("滑动音量默认21："+(volumes * 100)/15);
                int changePercent = (int) ((nowY - downY) * 100 / getHeight());
                if (mGestureDialogManager != null) {
//                    mGestureDialogManager.showVolumeDialog(AliyunVodPlayerView.this, volume * 100);
                    mGestureDialogManager.showVolumeDialog(AliyunVodPlayerView.this, ((volumes/15)*100));//zdd
//                    mGestureDialogManager.showVolumeDialog(AliyunVodPlayerView.this, (int) changePercent);
                    float targetVolume = mGestureDialogManager.updateVolumeDialog(changePercent);
                    if(onVolumUpdateListener != null){//zdd
                        onVolumUpdateListener.onVolumNum(targetVolume);
                    }
                    AppLog.e("1009:currentVolume:"+((targetVolume * 15)/100.0));
                    currentVolume = targetVolume;
                   // mAliyunVodPlayer.setVolume((targetVolume / 100.00f));//通过返回值改变音量
                    mAliyunVodPlayer.setVolume((targetVolume * 15)/100.0f);//zdd
                    AudioManagerUtl.getAudioManager().setStreamVolume(AudioManager.STREAM_MUSIC, (int) ((targetVolume * 15)/100.0),AudioManager.FLAG_PLAY_SOUND);
                }
            }
...
//            return mAliyunVodPlayer.getAllDebugInfo();
     ...
     * 设置播放器模式，自适应，填充
     */
    public void setScaleModel(IPlayer.ScaleMode model){
        if(mAliyunVodPlayer != null){
            mAliyunVodPlayer.setScaleMode(model);
        }
    }
    ...
    //音量调节 zdd修改
    public interface OnVolumUpdateListener{
        void onVolumNum(float volum);
    }

    public void setOnVolumClickListener(OnVolumUpdateListener listener){
        this.onVolumUpdateListener = listener;
    }
    ...
    private void sourceVideoPlayerPrepared() {
        ...
        mPlayingVideoId = mAliyunVodPlayer.getMediaInfo().getVideoId();
        //防止服务器信息和实际不一致
     ...
     * 播放器模式接口
     */
    public interface OnAliVodPlayScalModel{
        void onScalModel(IPlayer.ScaleMode mode);
    }
    private OnAliVodPlayScalModel aliVodPlayScalModel;

    public void setOnPlayVodScalListener(OnAliVodPlayScalModel scalListener){
        this.aliVodPlayScalModel = scalListener;
    }
    ...
    public void setVideoTitle(String title){
        mControlView.updateTitle(title);
    }
}
